...
def seed_database():
    ...
    # PROVJERA: Da li su podaci vec ubaceni?
    try:
        cursor.execute("SELECT COUNT(*) FROM users")
        count = cursor.fetchone()[0]
        if count > 0:
            print("Baza podataka je vec popunjena. Preskacem seed-ovanje.")
            conn.close()
            return
    except sqlite3.OperationalError:
        # Ako tabela ne postoji, db_init.py ce to rijesiti, ali za svaki slucaj
        print("Tabela 'users' ne postoji. Pokrenite db_init.py prvo.")
        conn.close()
        return
